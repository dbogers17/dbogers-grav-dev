...
export class RenderSystem{
 constructor(ctx,world){this.ctx=ctx;this.world=world;this.unified=new UnifiedFactionRenderer();this.space=new CalmInfiniteSpaceRenderer();this.assets=new AssetManager();this.station=new StableHelixRenderer(this.assets);this.traffic=[];this.rocks=world.asteroids.map((r,i)=>({...r,a:i*.8}))}
 ...
 drawWorld(camera,w,h,time=performance.now()){const c=this.ctx,planet=this.world.planet,p=camera.worldToScreen(planet.x,planet.y,w,h,.34),radius=planet.r*.38;c.save();c.fillStyle='#010103';c.beginPath();c.arc(p.x,p.y,radius*1.78,0,Math.PI*2);c.fill();c.restore();
  c.save();c.strokeStyle='rgba(210,225,235,.42)';c.lineWidth=3;c.beginPath();c.ellipse(p.x,p.y,radius*1.52,radius*.29,-.2,Math.PI,Math.PI*2);c.stroke();c.restore();this.unified.planet(c,'gas',p.x,p.y,radius*2.3);c.save();c.strokeStyle='rgba(225,235,242,.62)';c.lineWidth=3;c.beginPath();c.ellipse(p.x,p.y,radius*1.52,radius*.29,-.2,0,Math.PI);c.stroke();c.restore();
  for(const r of this.rocks){const q=camera.worldToScreen(r.x,r.y,w,h,1);if(q.x<-60||q.x>w+60||q.y<-60||q.y>h+60)continue;this.drawAsteroid(c,r,q)}
  for(const t of this.traffic){const q=camera.worldToScreen(t.x,t.y,w,h,1),a=Math.atan2(t.vy||0,t.vx||1);if(q.x>-100&&q.x<w+100&&q.y>-100&&q.y<h+100)this.unified.ship(c,t.role,q.x,q.y,a,Math.max(30,(t.size||20)*1.9))}
  for(const loc of this.world.locations){const q=camera.worldToScreen(loc.x,loc.y,w,h,1);if(q.x<-300||q.x>w+300||q.y<-300||q.y>h+300)continue;if(loc.type==='station'){this.station.draw(c,q.x,q.y,time);continue}c.strokeStyle=loc.type==='quest'?'#fbbf24':'#60a5fa';c.lineWidth=2;c.beginPath();c.arc(q.x,q.y,13,0,Math.PI*2);c.stroke();c.fillStyle='#dbeafe';c.font='12px sans-serif';c.textAlign='center';c.fillText(loc.name,q.x,q.y+38)}}
}
